...
export default function MonteCarloPage() {
  ...
  const handleApplyLiveData = useCallback((params: { S0: number; mu: number; sig: number }) => {
    s.set({ S0: params.S0, mu: params.mu, sig: params.sig })
  }, [s.set])
    ...
    <div className="flex h-screen flex-col">
      ...
      <ModelShell>
        ...
        <div className="relative min-w-0 flex-1">
          ...
          />
          <Hint text="Drag to rotate · Scroll to zoom · Right-click to pan · Paths sorted by terminal · [B] left · [R] right" />
        </div>

        {rightOpen && (
          <RightSidebar metricsContent={metricsContent} scaleLabel={s.metric === 'price' ? 'TERMINAL PRICE' : 'TERMINAL RETURN'} />
        )}
      </ModelShell>
